...
step: open `index.html` in a browser (works from `file://`). This site is the PUBLIC source of
truth for v1; it must never reference this (private) repo directly - no `demos/` paths, no
`main.rs` chips, no internal doc names. Covenant `.simf` file:line chips are fine (the covenant
source ships with the deployed artifacts, CMR-pinned).

- `index.html` .. `glossary.html` - the eleven chapters (page tree per `v1-SPEC-SITE-PLAN.md`).
...
Maintenance rules (maintainer-facing; the sources below are private):
...
- The spec is normative for the public, and it must stay in exact agreement with the frozen
  ...
  asserts and the `main.rs` builders in `demos/04-v1-cdp`.
